...
def triangle_side_length(vertices: dict) -> float:
    """Return the length of one edge (the triangle is equilateral)."""
    rx, ry = vertices["red"]
    gx, gy = vertices["green"]
...
def draw_centroid_marker(
    ...
    """
    Draw a filled circle at the triangle's centroid in the current mixed
    colour (r/3, g/3, b/3 - the centroid is equidistant from all three
    corners, so each corner contributes its full slider value at exactly
    one-third weight, matching apply_slider_colors()'s own per-pixel
    formula colour(x,y) = (lambda_R * r, lambda_G * g, lambda_B * b)).

    Sized as a fraction of the triangle's own side length rather than a
    fixed pixel count, so it stays proportionally readable after this
    game's native 720x720 render is downscaled to the wall's actual, much
    lower resolution (see render_wall_frame() in main.py) - a fixed small
    pixel radius would shrink to an unreadable handful of source pixels,
    and at typical wall sizes (64x64-128x128) that can be well under a
    single wall pixel.
    """
    cx, cy = triangle_centroid(vertices)
    radius = max(1, int(triangle_side_length(vertices) * 0.14))

    mixed = (int(r / 3), int(g / 3), int(b / 3))
    pygame.draw.circle(surface, mixed, (cx, cy), radius)
    pygame.draw.circle(surface, outline_color, (cx, cy), radius, width=max(1, radius // 12))